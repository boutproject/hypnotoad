...
def create_tokamak(geometry="sn", nx=65, ny=65):
    ...
    psi_functions = {
        "lsn": lambda R, Z: (np.exp(-((R - r0)**2 + (Z + z0 - 0.3)**2)/0.3**2) +
                            np.exp(-((R - r0)**2 + (Z + z0 + 0.3)**2)/0.3**2)),
        "usn": lambda R, Z: (np.exp(-((R - r0)**2 + (Z - z0 - 0.3)**2)/0.3**2) +
                            np.exp(-((R - r0)**2 + (Z - z0 + 0.3)**2)/0.3**2)),
...
if __name__ == "__main__":
    ...
    r1d, z1d, psi2d = create_tokamak(geometry = options.get("geometry", "lsn"),
                                     nx = options.get("nx", 65),
                                     ny = options.get("ny", 65))
    
    from hypnotoad2 import tokamak
    
    eq = tokamak.TokamakEquilibrium(r1d, z1d, psi2d,
                                    [], [], # psi1d, fpol
                                    options=options)
