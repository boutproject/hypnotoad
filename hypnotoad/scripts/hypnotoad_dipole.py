#!/usr/bin/env python
#
# This script generates a BOUT++ grid file for a dipole flux surface
# geometry using a dipoleq equilibrium parsed with DipoleEqReader, optionally using a set of inputs in a YAML file
#
# For example:
#  $ hypnotoad-dipole settings.yml
#
...
def get_arg_parser():
    ...
    parser = ArgumentParser(
        description="""
        Create a grid file for a configuration for dipolar concentric flux surfaces.
        """
    )
...
def main(*, add_noise=None):
    ...
    from ..cases.dipole import DipoleEquilibrium
    from ..core.mesh import BoutMesh

    possible_options = (
        [opt for opt in DipoleEquilibrium.user_options_factory.defaults]
        + [opt for opt in DipoleEquilibrium.nonorthogonal_options_factory.defaults]
